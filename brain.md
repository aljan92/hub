...
- Wiederholung erfordert genau einen vorhandenen Queue-Eintrag in WAITING oder ERROR und einen abgeschlossenen/fehlgeschlagenen Task. Laufende, abgeschlossene oder remote ungeklärte Uploads sind gesperrt. Es wird weder neu eingereiht noch explizit ein Upload gestartet. Der vorhandene Queue-Status inklusive Pause bleibt erhalten; eine aktive automatische Queue kann nach Freigabe wieder regulär weiterarbeiten.
...
- Der Zwischenfix `c668ab9` (4.000 Stempel/vier Pässe) behob den gemeldeten PNG-Stillstand nicht. Diese Optikänderung wurde deshalb zurückgenommen: wieder ursprüngliche zehn Pässe und 50%-Stempelverteilung, weiterhin deterministisch. Nachgewiesene neue Zusatzarbeit war die vollständige Datenadressenschleife zur Seed-Berechnung; auch die erneute PNG-Decodierung über SVG war unnötig. Nun direkter Brush-Maskenzuschnitt aus dem bereits geladenen Bild und 32-Bit-Seed aus dem vorhandenen Render-Fingerprint. Keine Trimmed-Datei, kein Einfluss auf PNG-No-Upscale oder SVG-Vordergrund. Fingerprint-Version erhöht, damit alte Ausgaben nicht als aktuell gelten.
- Brush-Teilschrittzeiten (`sourceMs`, `brushLoadMs`, `cleanupMs`, `contourMs`, `encodeMs`, `totalMs`) und Stempelzahlen werden im Prompt Log und Serverlog gemeldet. Neuer Test `pngBrushRegression.test.ts`: detailreiches PNG mit rund 11 Mio. Datenadressenzeichen, transparenten Konturen, keine `src`-Getterabfragen oder Source-Re-Decodierung erlaubt, reproduzierbarer kurzer Seed. Lokal rund 3,1 Sekunden Brush-Vorbereitung beim Testmotiv; keine Aussage zur konkreten NAS-Datei, die lokal nicht vorliegt.
- Tests/Abnahmestand im [Umbauplan](./RESIZE_SVG_PLAN.md). Lokaler Master-Test pixelgleich, acht Varianten in voller Größe erfolgreich. PNG- und SVG-Resize-Pfade sowie großer Blanket-Upload wurden inzwischen praktisch bestätigt; Metriken im [Performance-Audit](./PERFORMANCE_AUDIT.md).

### 10.57 PNG-Ausgabe nach Blanket-EncodingError (4. September 2026)

- Nutzerlog: Brush erfolgreich (29,3 s), vier Two-Sided-Varianten erfolgreich, danach `EncodingError: The source image cannot be decoded` bei Blanket 8904×10584. Der alte Ausgabeweg unterschied SVG-Kompositionsdecode und anschließenden PNG-Prüfdecode nicht. Exakte NAS-Ursache lokal nicht reproduziert; Ressourcenproblem plausibel, nicht bewiesen.
- PNG-Varianten zeichnen jetzt direkt aus dem bereits geladenen Original auf einen Ziel-Canvas. Keine große SVG-Hülle/Viewport-Screenshot-Kette für PNG. Gemeinsame Platzierungsberechnung mit SVG; Motiv höchstens 1:1, nur bei Bedarf verkleinern. Canvas nach Kodierung freigegeben. SVG bleibt direkte Vektorkomposition; Master unverändert. Keine Produkt-Sonderfälle, Pfade/IDs/Zielmaße unverändert.
- Variantenprüfung ersetzt den zweiten Browser-Vollbilddecode durch `artworkPngValidation.ts`: PNG-Struktur/CRC/Maße, vollständiges stückweises Zlib-Entpacken, erwartete Scanline-Länge und gültige Filter. Unterstützt bewusst nur die nicht-interlaced 8-Bit-RGB/RGBA-Ausgabe unserer Browser-Encoder; kein allgemeiner PNG-Decoder und keine visuelle Inhaltsprüfung. 300-DPI-Metadaten ersetzen bestehendes pHYs. Prüfung vor atomarer Dateiübernahme; Queue-Schutz unverändert.
- Fehler enthalten Variante, Maße, Quelle und Teilschritt (`PNG_CANVAS_RENDER_ENCODE`, `SVG_COMPOSITION_DECODE`, `SVG_SCREENSHOT`, `PNG_INTEGRITY_VALIDATION` etc.). Renderer-Fingerprint auf v5 erhöht; alte Dateien nicht gelöscht.
- Neue Tests: `artworkPngValidation.test.ts` (Korruption, Chunk-Reihenfolge, Filter, Zlib, Dichte, Datei-Erhalt bei Fehler); `pngCanvasRegression.test.ts` (No-Upscale/Downscale, Brush-Positionen; mit `ARTWORK_FULL_SIZE_TEST=1` 4065×3720 Motiv in 4500×5400 PNG plus sämtliche Produktionsgrößen). Vollständiger SVG-Test, Finalisierungs-/Wiederholungs-Tests, Architektur-Guard und Build bestanden. NAS-Abnahme über Dashboard-Update und erneute Finalisierung bleibt offen.

### 10.58 Finalisierung einzeln nach Fehler wiederholen (4. September 2026)

- Ursache des ausgegrauten Buttons: U7-Fehler behielten `UPDATE_TRANSLATED` mit `hasError`, während die Oberfläche nur COMPLETED/UPDATE_QUEUED/ERROR zuließ. Der Server verlangte zudem bereits einen Queue-Eintrag, der bei fehlgeschlagener erster Artwork-Vorbereitung noch nicht existiert.
- Gemeinsame UI-/Server-Freigabe in `src/types/finalizationRetry.ts`; fehlgeschlagener letzter Finalisierungsevent erlaubt Wiederholung auch im alten U7-Zustand. Offene Reviews/Checkpoints und aktive Task-Locks bleiben gesperrt.
- Ohne Queue-Eintrag ist ausschließlich die vor erster Übergabe fehlgeschlagene Finalisierung wiederholbar. Frühere erfolgreiche Übergaben (`TASK_HANDOFF`), `inQueue`, doppelte Einträge, beschädigter Queue-Speicher und Remote-Unklarheit verhindern eine unsichere Wiederholung. Kein Wiederanlegen bereits entfernter Upload-Aufträge.
- Normaler Workflow und manuelle Wiederholung verwenden dieselben nebenwirkungsfreien Parameter-Builder. Nur vorhandenes Listing bereinigen/validieren und Druckdateien aus vorhandener PNG-/SVG-Quelle neu erzeugen; keine Analyse, LLM-Generierung, Übersetzung oder Vektorisierung. Neue Generation-ID, prepareOnly; vor erster Übernahme Task-Eingaben und Queue nochmals prüfen. Gemeinsamer synchroner `handoffPrepared` vollendet anschließend die erste Queue-Übergabe, ohne ein zweites Rendering.
- Mit vorhandenem WAITING-/ERROR-Eintrag bleibt der bisherige sichere Dateiaustausch bestehen. Der Button startet selbst keinen Upload, eine aktive automatische Queue kann nach erfolgreicher Freigabe regulär weiterarbeiten; Bestätigungsdialog weist darauf hin.
- Erweiterte `manualFinalization.test.ts`: Update-/Design-Fälle ohne Queue, alter UPDATE_TRANSLATED-Fehlerstatus, Parametererhalt, Reviews, entfernte frühere Aufträge, Renderfehler, konkurrierende Änderungen/Queue-Einträge, vorhandene Remote-/Uploadguards. Finalisierungsintegration, Unified-Finalisierung, Architektur-Guard und Build bestanden; bekannter allgemeiner TypeScript-Fehlerbestand weiterhin vorhanden.

### 10.59 TM-Review-Freigabe nach SQLite-Umstellung (4. September 2026)

- Ursache des Review-Loops isoliert reproduziert: `submitTmReview` änderte nur ein aus SQLite rekonstruiertes Task-Objekt und schrieb anschließend frisch geladene, unveränderte Logs zurück. Das Freigabe-Event wurde gespeichert, Status/Checkpoint/Listing hingegen nicht. Zusätzlich brach der Design-Pfad an nicht definiertem `settings` ab.
- APPROVE/REJECT speichern gezielt den betroffenen Task über `updateTaskStatus`, bevor Erfolg gemeldet wird. Freigabe entfernt den TM-Checkpoint, erhält bearbeitetes Listing und Produktsperren und setzt den tatsächlichen Folgestatus. Übersetzungen werden ebenfalls gezielt persistiert; kein globales `saveLogs(loadLogs())` mehr in dieser Aktion.
- Settings korrekt laden; Designs fahren mit Übersetzung (wenn aktiviert) und Vektorisierung fort, Updates ausschließlich ab U6 mit dynamischem Import. Kein erneuter TM-Audit bei APPROVE; RECHECK bleibt explizite eigene Aktion.
- Task-Lock bis Ende der Fortsetzung, Statusprüfung gegen doppelte/veraltete Freigaben. Update-Fortsetzung nutzt denselben Lock-Owner. Folgefehler werden als ERROR mit entferntem Review-Checkpoint dokumentiert; kein stiller Vektorisierungs-Fallback bei fehlgeschlagener Übersetzung.
- `tests/tmReviewPersistence.test.ts` nutzt echte isolierte SQLite-Persistenz und gemockte externe Schritte: Freigabe/Listing/Sprachen, deaktivierte Übersetzung, Ablehnen, Recheck, Doppelklick, Update-U6, Speicher- und Übersetzungsfehler. Test, bestehende manuelle Finalisierungsregression, Architektur-Guard und Build bestanden. Keine bestehenden Nutzertasks automatisch verändert; nach Dashboard-Update betroffenen Task erneut freigeben.

### 10.60 Cutout-Freigabe → Finalisierung → Queue (4. September 2026)

- Zweiter bestätigter Übergabefehler: `updateTaskStatus(COMPLETED)` setzte vorher `inQueue=true`, bevor `completeTaskAndEnqueue` startete. Dieses brach deshalb sofort ab. SVG-/Master-Pfade und Auditdaten waren ebenfalls nur an abgetrennten Task-Objekten geändert worden. Die Oberfläche meldete schon für COMPLETED pauschal Queue-Erfolg.
- Automatische und manuelle Cutout-Freigabe persistieren SVG, Vorschau, Audit und Master-PNG gezielt. Anschließend wird die Finalisierung abgewartet; erst echte Queue-Übernahme setzt COMPLETED/inQueue. Zwischenstatus `FINALIZING` mit passender Anzeige und Recovery-Fortsetzung direkt ab Finalisierung. Fehler bleiben ERROR statt falschem Erfolg; D7/D8 und der Enqueue-Endpunkt reichen Fehler korrekt weiter.
...
- `tests/uploadDailyLimit.test.ts` prüft Draft-Ausnahme und Publish-/Update-Abgrenzung ohne echte Uploads. Praktischer Amazon-Draft-Test nach Deployment am 5. September erfolgreich: Tageslimit-Hinweis wurde im Draft-Modus übergangen und der Ablauf erreichte Save Draft.

### 10.62 Draft-Speicherung und großer SVG-Blanket-Abriss (5. September 2026)

- Draft-Abbruch `remoteBaseline is not defined`: Recovery-Fingerprint und optionale Update-Baseline waren fälschlich innerhalb des Publish-Zweigs deklariert, wurden aber auch vor Save Draft persistiert. Beide Werte nun vor der Modusverzweigung berechnen; Draft erhält `undefined` als Baseline, aber denselben dauerhaften Remote-Intent-Schutz. Verzweigung verwendet den bereits bestimmten `effectiveMode`. Save-Draft-Button/Netzwerkbestätigung unverändert.
- Reale Datei `_068-H_canvas_bg_contain_4452x5292_dark.png` geprüft: korrekte 8904×10584-PNG-Struktur, aber ab ca. Zeile 3900 vollständig transparent. Damit war Amazon nicht Ursache der unvollständigen Vorschau. Der alte SVG-Pfad fotografierte einen 8904×10584-Chromium-Viewport; die übergroße Screenshot-Oberfläche riss ab, während Header/CRC/Zlib formal gültig blieben.
- SVG-Varianten werden nun wie PNG-Varianten aus der vollständig decodierten Komposition auf einem Ziel-Canvas kodiert, nicht als Viewport-Screenshot. Zielmaße, Vektorquelle, Profile und No-Upscale-Regel für PNG unverändert. Hintergrundprofile prüfen alle vier Ausgabeecken vor Kodierung auf Deckkraft und verweigern transparente Abrisskanten. Anschließend bleibt die bestehende vollständige PNG-Integritätsprüfung aktiv. Fingerprint v6 invalidiert fehlerhafte v5-Cachedateien.
- Exakt das reale 332-KB-SVG lokal neu als Blanket gerendert: 8904×10584, ca. 4,74 MB, alle Ecken sowie frühere Abrisszeile deckend; Motiv vollständig sichtbar. Variante ca. 1,0 s, isolierter Job ca. 1,8 s/333 MiB gesampeltes Chromium-Prozessbaum-RSS auf Entwicklungs-Mac; kein NAS-Leistungsversprechen.
- Vollständiger Acht-Varianten-SVG-Test einschließlich großer deckender Blanket-Ecken bestanden. `uploadDailyLimit.test.ts` prüft zusätzlich, dass Draft-Recoverywerte vor Publish/Draft-Verzweigung deklariert sind. Build und Architektur-Guard bestanden. Praktische Abnahme am 5. September ebenfalls erfolgreich: neu erzeugte Resize-Varianten inklusive Blanket vollständig und Draft-Upload erfolgreich gespeichert. Alte v5-Dateien werden weiterhin nicht automatisch gelöscht.

### 10.63 Praktischer Abschluss Resize-/Draft-Korrekturen (5. September 2026)

- Nutzer bestätigt nach Deployment von `759aee2`: „nun klappt es perfekt“. Der Draft-Ablauf speichert trotz ausgeschöpftem Tageslimit, und die neu erzeugten Resize-Varianten einschließlich Blanket werden vollständig dargestellt.
- Damit sind PNG-No-Upscale, SVG-Direktvarianten, 200%-Blanket, großer SVG-Canvas-Renderer, manuelle Finalisierungswiederholung und Draft-/Hybrid-Tageslimit-Ausnahme praktisch abgenommen. Diese Bestätigung gilt für die getesteten Designs und ersetzt keine allgemeine Garantie für beliebige zukünftige Amazon-DOM- oder SVG-Änderungen.
- Resize-/Draft-Paket aktuell abgeschlossen. Bei späteren Performance-Arbeiten bei [PERFORMANCE_AUDIT.md](./PERFORMANCE_AUDIT.md) und [RESIZE_SVG_PLAN.md](./RESIZE_SVG_PLAN.md) fortsetzen; nicht bei null beginnen.

### 10.64 Nizza-Klassen-Anzeige und persistente Null-Zuordnung (5. September 2026)

- Ursache der pauschalen Anzeige „Klasse 25“ war kein nachgewiesener Verlust der gespeicherten Overrides, sondern der UI-Ausdruck `niceClass || 25` an vier Stellen. Dadurch wurden `null` und fehlende Werte optisch als Klasse 25 ausgegeben.
- Produktliste, Detailkopf und Konfigurationskarte zeigen nicht zugeordnete Produkte nun ausdrücklich als „Nicht zugeordnet“. Das Dropdown unterstützt `null`; es wird keine Klasse geraten. Bestehende numerische Overrides werden unverändert angezeigt.
- Die API akzeptiert ausschließlich `null` oder eine ganzzahlige Nizza-Klasse 1–45. Die Oberfläche prüft den HTTP-Erfolg, übernimmt den vom Server zurückgegebenen Katalog und rollt eine optimistische Anzeige bei Speicherfehler zurück, statt eine nur scheinbar gespeicherte Klasse stehen zu lassen.
- Speicherung bleibt ausschließlich in `product_catalog_overrides.json`; Scanner-Daten überschreiben die manuelle Zuordnung nicht. Regressionstest setzt einen bestehenden Override auf `null`, simuliert einen Prozessneustart, prüft den Ausschluss aus der Klassenblockierung und stellt die ursprüngliche Klasse anschließend wieder her.
- Der praktisch abgenommene Resize-Punkt wurde aus `Alex Todo.md` in `Alex Todo Completed.md` verschoben. Die separate NAS-Laufzeit-/Speichermessung bleibt weiterhin offen.
