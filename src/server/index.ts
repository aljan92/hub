...
app.post('/api/v1/tasks/:taskId/evaluate-field-tm', async (req, res) => {
  const { taskId } = req.params;
  const { field, text, hits } = req.body;
  try {
    if (!field || typeof text !== 'string') {
      return res.status(400).json({ success: false, error: 'Feld und Text sind erforderlich.' });
    }
    ...
    const result = await TaskLogService.evaluateSingleFieldTm(taskId, field as any, text, hits);
    res.json({ success: true, ...result });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
...
app.post('/api/v1/tasks/:taskId/rewrite-field-tm', async (req, res) => {
  const { taskId } = req.params;
  const { field, text, hits, fairUseEvaluation } = req.body;
  try {
    if (!field || typeof text !== 'string') {
      return res.status(400).json({ success: false, error: 'Feld und Text sind erforderlich.' });
    }
    ...
    const result = await TaskLogService.rewriteSingleFieldTm(taskId, field as any, text, hits, fairUseEvaluation);
    res.json({ success: true, ...result });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
