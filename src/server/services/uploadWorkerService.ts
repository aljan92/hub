        ...
        const editResult = await page.evaluate(async (params: {
          ...
          if (params.colorMode === 'customPicker') {
            ...
            for (const cb of colorCheckboxes) {
              ...
              // 1. Check if this color is allowed according to product catalog definition
              let isCatalogAllowed = true;
              if (params.catalogColors && params.catalogColors.length > 0) {
                isCatalogAllowed = params.catalogColors.some((ac: string) => {
              ...
              // 3. Accurately detect if swatch is currently checked on Amazon Merch DOM
              ...
              const isChecked = Boolean(
                ...
                cb.querySelector('.selected, .checked, [aria-checked="true"]') !== null ||
                hasCheckIcon ||
                (cb.querySelector('input') as HTMLInputElement)?.checked
              );
