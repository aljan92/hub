...
function previewContent(value: unknown, depth = 0, isRequest = false): unknown {
  ...
  if (Array.isArray(value)) return value.slice(0, 40).map(item => previewContent(item, depth + 1, isRequest));
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    result[key] = previewContent(child, depth + 1, isRequest);
  }
  return result;
}
...
export function projectPromptLogTask(task: DesignTaskLog): DesignTaskLog {
  const events = task.events.map(event => ({
    ...event,
    content: event.type === 'LLM_RESPONSE' && typeof event.content === 'string'
      ? '[Antwort auf Anforderung]'
      : previewContent(event.content, 0, event.type.endsWith('_REQUEST'))
  }));
