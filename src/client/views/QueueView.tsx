...
export const QueueView: React.FC = () => {
  ...
  const [catalogProducts, setCatalogProducts] = useState<Array<{ id: string; amazon?: { key: string } }>>([]);
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/products/catalog')
      .then(r => r.json())
      .then(d => {
        if (d.success && d.catalog?.products) {
          setCatalogProducts(d.catalog.products);
        }
      })
  ...
  // Helper to normalize Amazon product keys to exact catalog product IDs via dynamic catalog
  const normalizeCatalogProductId = (raw: string): string => {
    const s = String(raw || '').trim().toUpperCase().replace(/[^A-Z0-9]/g, '_').replace(/_+/g, '_');
    const matched = catalogProducts.find(p => p.id.toUpperCase() === s || p.amazon?.key?.toUpperCase() === s);
