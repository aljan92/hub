...
Ressourcen: pro Variante Maße/Laufzeit/PNG-Bytes; pro Job Laufzeit und im 1-s-Takt gesampeltes maximales aggregiertes Chromium-Prozessbaum-RSS (MiB). Shared Memory kann mehrfach gezählt werden, kurze Spitzen können fehlen. Getrennt von Upload-/Screencast-Sessions. Zehn-Minuten-Joblimit, Browser-Abbau bei Fehlern, 100-MP-Pixelbudget. Nach erfolglosem Zwischenfix sind die ursprünglichen Brush-Parameter wiederhergestellt. Stattdessen wird der direkte Source-Crop im Speicher genutzt, mit kurzem Fingerprint-Seed ohne Datenadressenschleife. Brush-Teilschrittzeiten und Stempelzahlen erscheinen im Prompt Log. `pngBrushRegression.test.ts` deckt diesen Pfad mit einem detailreichen PNG ab. NAS-Vorher-/Nachher-Benchmark und praktische Abnahme stehen aus; Todo 4/5 noch nicht vollständig abschließen.

Nachtrag zum Blanket-EncodingError: PNG-Ausgabe nun direkt per Canvas mit derselben Profil-/Platzierungsberechnung wie SVG, ohne erneute SVG-Kompositionsdecodierung und Screenshot. Kein PNG-Upscaling. Varianten werden vor Dateiübernahme speichersparend anhand von PNG-Struktur, CRC, Maßen und vollständig gestreamtem Zlib-/Scanline-Inhalt geprüft; dies ersetzt hier die ursprünglich geplante zweite Browser-Bilddecodierung, nicht durch eine reine Header-Prüfung. Kein allgemeiner PNG-Decoder oder visueller Inhaltsvergleich. Fehler melden die genaue Phase und Variante. `artworkPngValidation.test.ts` und `pngCanvasRegression.test.ts` einschließlich vollständiger PNG-Produktionsgrößen bestanden; bisheriger vollständiger SVG-Test weiterhin bestanden. Exakter NAS-Fehler lokal nicht reproduziert; Vergleichswerte im Performance-Audit und erneuter Nutzertest erforderlich.
