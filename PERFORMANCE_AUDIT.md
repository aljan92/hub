...
- Große Jobs global serialisiert. Isolierter Chromium-Prozess wird nach jedem Job beendet. PNG-Varianten inzwischen direkt auf Canvas statt über SVG-Hülle/Viewport-Screenshot. Variantenprüfung durch stückweises Zlib-Entpacken mit Struktur-/CRC-/Scanline-Prüfung statt zweitem Browser-Vollbilddecode. Rasterquelle, Brush-Ebene und Canvas-Übertragung verwenden noch Data-URIs/Base64; nicht sämtliche Kopien sind entfernt.
...
- Historischer synthetischer Vollgrößenlauf mit der inzwischen zurückgenommenen Brush-Begrenzung: acht Varianten, etwa 5,4 Sekunden für den Batch und rund 1,21 GiB gesampeltes Prozessbaum-RSS. Nicht mit einem realen PNG-Brush-Benchmark verwechseln. Werte schwanken mit Chromium-Speicherbereinigung und sind keine Prognose für NAS.
- Korrektur nach weiterem PNG-Stillstand: alte Konturparameter wiederhergestellt, aber neue Voll-Datenadressenschleife entfernt. PNG-Brush bezieht einen direkten Crop aus dem geladenen Bild statt erneuter Decodierung über eine SVG-Hülle; kurzer Seed aus bestehendem Fingerprint. Im lokalen Diagnosemotiv kostete die vorherige Seed-Schleife allein 2,82 s gegenüber 0,44 s für die eigentliche Brush-Berechnung. Das belegt eine Regression, nicht allein die Ursache des fünfminütigen NAS-Stillstands.
- Neuer Regressionstest mit ca. 11 Mio. URI-Zeichen und transparenten Konturen: direkte PNG-Brush-Vorbereitung ca. 3,1 s lokal. Teilschritt-Metriken werden im Prompt Log ausgegeben. Der Test verbietet Source-Re-Decodierung und `image.src`-Getterzugriffe, statt nur einen maschinenabhängigen Zeitgrenzwert zu prüfen. Original-PNG/NAS-Abnahme weiterhin erforderlich. Blanket besitzt gegenüber ursprünglichen Maßen viermal so viele Pixel.
...
- Nach Blanket-`EncodingError`: isolierter synthetischer PNG-Vergleich mit 4065×3720 sichtbarem Motiv in 4500×5400 Quelle und 8904×10584 Ausgabe. Alter Pfad lokal erfolgreich, ca. 12,35 s/1378 MiB Chromium-Prozessbaum-RSS; direkter Canvas-Pfad ca. 4,9–5,1 s/818–931 MiB, darin eigentliche Variante ca. 2,4–2,5 s. Keine identische NAS-Datei, kein Nachweis der ursprünglichen Fehlerursache. Vollständiger neuer Acht-Varianten-PNG-Job ca. 13,4 s/1184 MiB. Synthetische rechteckige Alpha-Grenze ergab null Brush-Stempel und ist kein Ersatz für den separaten Konturtest oder den realen 29,3-s-Brush-Lauf. SVG-Vollgrößenlauf ebenfalls bestanden; große Vektor-Viewports bleiben ressourcenintensiv.
- Reale SVG-Blanket-Datei aus v5 zeigte einen transparenten Screenshot-Abriss ab etwa Zeile 3900 trotz formal gültiger PNG-Struktur. SVG-Ausgaben nutzen ab v6 Canvas-Kodierung statt großer Viewport-Screenshots; Hintergrundprofile prüfen deckende Ecken. Exaktes 332-KB-Nutzer-SVG: vollständige 8904×10584-Ausgabe ca. 4,74 MB, Variante lokal ca. 1,0 s, isolierter Job ca. 1,8 s/333 MiB Prozessbaum-RSS. Synthetischer vollständiger Acht-Varianten-SVG-Test ca. 4,1 s/712 MiB plus separate Inhaltsprüfung. Werte sind nicht direkt mit NAS/realem Komplettbatch inklusive dessen 22,5-s-Brush vergleichbar.
